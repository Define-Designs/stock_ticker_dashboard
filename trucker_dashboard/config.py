...
# --- Options contract selection (yfinance) ---
...
# Yahoo (via yfinance) doesn't publish greeks, so options_provider.py
# computes delta itself with the standard Black-Scholes formula using
# yfinance's implied volatility. This is the annualized risk-free rate that
# formula needs (approximately the current short-term T-bill yield) - it's a
# minor input that barely moves delta, so it doesn't need to track real
# rates day to day.
RISK_FREE_RATE = 0.05
